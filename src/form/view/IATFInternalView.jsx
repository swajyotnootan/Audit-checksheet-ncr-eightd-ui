      ...
      if (checkSheetId) {
        try {
          ...
          const sheetRes = await axios.get(
            `http://localhost:8080/api/templates/${checkSheetId}`,
            { withCredentials: true }
          );
  ...
  const fetchQuestionsByDepartment = async (department, process) => {
    ...
    try {
      const checkSheetRes = await axios.get(
        `http://localhost:8080/api/templates/iatf/by-department/${encodeURIComponent(department)}`,
        { withCredentials: true }
      );
      ...
      if (sheets && sheets.length > 0) {
        ...
        const sheetDetailsRes = await axios.get(
          `http://localhost:8080/api/templates/${selectedSheet.id}`,
          { withCredentials: true }
        );
