...
async sendTo8D(id, comment, userId) {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`http://localhost:8080/api/ncr/${id}/send-to-8d`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
...
submitNCR2: async (ncrId, actionData) => {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`http://localhost:8080/api/ncr/${ncrId}/submit-ncr2`, {
            method: 'POST',
            headers: {
...
getPendingNCR2: async () => {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`http://localhost:8080/api/ncr/ncr2-pending`, {
            headers: {
                'Authorization': `Bearer ${token}`,
            },
        });
...
verifyNCR2: async (ncrId, accepted, comment) => {
  try {
    const token = localStorage.getItem('token');
    const response = await fetch(`http://localhost:8080/api/ncr/${ncrId}/verify-ncr2`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
