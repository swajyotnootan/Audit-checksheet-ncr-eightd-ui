...
export const TopNavBar = ({ user, onLogout }) => {
    ...
    const getProfilePhotoUrl = (user) => {
        if (user?.id && user.profilePhotoPath && !profilePhotoError) {
            return `https://internalaudit.hub.swajyot.co.in:8090/api/users/${user.id}/profile-photo`;
        }
        return null;
    };
    ...
    const profilePhotoUrl = userId ? `https://internalaudit.hub.swajyot.co.in:8090/api/users/${userId}/profile-photo` : null;
